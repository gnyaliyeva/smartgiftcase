...
export const themeSetter = () => {
  ...
  const colorName = window.localStorage.getItem("color");
  const sizeName = window.localStorage.getItem("size") || "--size3";

  window.localStorage.setItem("theme", themeName);
  colorName && window.localStorage.setItem("color", colorName);
  window.localStorage.setItem("size", sizeName);

  root.style.setProperty("--background", `var(${themeName}-bg)`);
  root.style.setProperty(
    "--color",
    colorName ? `var(${colorName})` : `var(${themeName}-color)`
  );
  root.style.setProperty(
    "--button",
    colorName ? `var(${colorName})` : `var(--primary)`
  );
...
export const handleTheme = (e) => {
  ...
  root.style.setProperty("--button", `var(--primary)`);
...
export const handleColor = (e) => {
  ...
  root.style.setProperty("--button", `var(--${e.target.id})`);
